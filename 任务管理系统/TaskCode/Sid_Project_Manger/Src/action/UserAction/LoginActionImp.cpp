...
{
	//因为已经获取到了用户的信息，所以可以获取用户的部门信息，就是数据库表相互联系
	...
	{//加载角色
		...
		if ( m_role.GetId() <= 0 )
		{
			ret = m_pUserInc->GetRoleByUserId( m_loginUser.GetId(), m_role );				//通过用户的ID获取用户的权限信息
		}
	    if ( m_role.GetId() > 0 )
	    {
			ret = m_pUserInc->GetUserPowersByRoleId( m_role.GetId(), m_powerList );			//获取这个等级的用户所有能使用的权限\
			它是通过用一个表，里面有索引号，用户类型代号，用户权限代号
	    }
		m_pUserInc->GetGernalManger( m_generManger );										//获取副总信息
		GetUserByPower( taskAgree, m_publishUser );											//通过权限反推用户的职称：“任务发布”
	}
    return ret;
}
