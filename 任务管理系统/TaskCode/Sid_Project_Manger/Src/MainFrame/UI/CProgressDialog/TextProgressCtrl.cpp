...
CTextProgressCtrl::CTextProgressCtrl()
{
	...
	m_bShowPercent = TRUE;
	m_dwTextStyle = DT_CENTER;			//文本居中

	// default marquee bar size of 20% of full progress bar
	m_nMarqueeSize = 20;				//默认滚动条的长度是20
}
...
ON_MESSAGE(TPBM_GETBARCOLOR, OnGetBarColor)
ON_MESSAGE(TPBM_GETBKCOLOR, OnGetBarBkColor)
...
void CTextProgressCtrl::OnPaint()
{
	...
	CTPBMemDC dc(&PaintDC);						// memory device context

	// get colors to use
	COLORREF crBarColor		= (COLORREF)OnGetBarColor(0, 0);				//获取已经使用的各个段的颜色信息
	...
	double dFraction = (double)(m_nPos - m_nMin) / ((double)(m_nMax - m_nMin));		//当前进度的百分比

	// get the control style
	DWORD dwStyle = GetStyle();				//获取进度条的样式属性
	...
	if (dwStyle & PBS_MARQUEE)				//有滚动条样式
	{
		if (dwStyle & PBS_VERTICAL)			//垂直字体样式
		{
			//有一种感觉就是你这个进度条是一根垂直的
			...
			//对进度条区域进行限定，不让越限
			if (MiddleRect.bottom >= LeftRect.bottom)				//如果进度条的前面一部分超过了进度条的长度			bleft = false
			{
				bLeft = false;
			...
			if (MiddleRect.top <= RightRect.top)				//我知道意思是想表达有一部分在里面
			{
				bRight = false;
	...
	if (str.GetLength())
	{
		...
		CSize szText;
		//如果是垂直样式的
		if (dwStyle & PBS_VERTICAL)		//如果是垂直放置
		{
...
void CTextProgressCtrl::OnTimer(UINT_PTR nIDEvent)			//起到滚动的效果
{
...
void CTextProgressCtrl::CreateVerticalFont()
{
	...
	lf.lfEscapement = lf.lfOrientation = 900;			//lfEscapement：以十分之一度为单位指定每一行文本输出时相对于页面底端的角度。
