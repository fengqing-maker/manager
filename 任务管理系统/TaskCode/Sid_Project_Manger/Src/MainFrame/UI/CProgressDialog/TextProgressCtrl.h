...
//		call	GetBarColor()
//		or		SendMessage (TPBM_GETBARCOLOR, 0, 0)
//		or		::SendMessage (m_hwnd, TPBM_GETBARCOLOR, 0, 0)
//
...
#define TPBM_GETBARCOLOR		(WM_USER+100)
#define TPBM_GETBKCOLOR			(WM_USER+101)
	...
	public:
		inline COLORREF SetBarColor(COLORREF crBarClr = CLR_DEFAULT)
			{ 
				...
				return ((COLORREF) ::SendMessage(m_hWnd, PBM_SETBARCOLOR, 0, (LPARAM)crBarClr));		//设置进度条指示栏的颜色
			}
		inline COLORREF GetBarColor() const
			{ 
				ASSERT(::IsWindow(m_hWnd)); 
				return ((COLORREF) ::SendMessage(m_hWnd, TPBM_GETBARCOLOR, 0, 0));		//获取进度条指示栏的颜色（我觉得就是中间的那个颜色）
			}
		inline COLORREF SetBarBkColor(COLORREF crBarBkClr = CLR_DEFAULT)				//设置进度条指示栏的背景颜色
			{ 
				ASSERT(::IsWindow(m_hWnd)); 
				return ((COLORREF) ::SendMessage(m_hWnd, PBM_SETBKCOLOR, 0, (LPARAM)crBarBkClr)); 
		}
		inline COLORREF GetBarBkColor() const					//获取进度条指示栏的背景颜色
			{ 
				ASSERT(::IsWindow(m_hWnd)); 
				return ((COLORREF) ::SendMessage(m_hWnd, TPBM_GETBKCOLOR, 0, 0)); 
		}
		inline COLORREF SetTextColor(COLORREF crTextClr = CLR_DEFAULT)		//设置进度条的文本颜色
			{ 
				ASSERT(::IsWindow(m_hWnd)); 
				return ((COLORREF) ::SendMessage(m_hWnd, PBM_SETTEXTCOLOR, 0, (LPARAM)crTextClr)); 
		}
		inline COLORREF GetTextColor() const				//获取进度条的文本颜色
			{ 
				ASSERT(::IsWindow(m_hWnd)); 
				return ((COLORREF) ::SendMessage(m_hWnd, PBM_GETTEXTCOLOR, 0, 0)); 
		}
		inline COLORREF SetTextBkColor(COLORREF crTextBkClr = CLR_DEFAULT)		//设置进度条文本的背景颜色
			{ 
				ASSERT(::IsWindow(m_hWnd));
				return ((COLORREF) ::SendMessage(m_hWnd, PBM_SETTEXTBKCOLOR, 0, (LPARAM)crTextBkClr)); 
		}
		inline COLORREF GetTextBkColor() const
			{ 
				ASSERT(::IsWindow(m_hWnd)); 
				return ((COLORREF) ::SendMessage(m_hWnd, PBM_GETTEXTBKCOLOR, 0, 0)); 
		}
		inline BOOL SetShowPercent(BOOL bShow)//是否显示进度条的百分比
			{ 
				ASSERT(::IsWindow(m_hWnd));
				return ((BOOL) ::SendMessage(m_hWnd, PBM_SETSHOWPERCENT, (WPARAM)bShow, 0)); 
		}
		inline DWORD AlignText(DWORD dwAlignment = DT_CENTER)				//设置进度条文本对齐方式
			{ 
				ASSERT(::IsWindow(m_hWnd));
				return ((DWORD) ::SendMessage(m_hWnd, PBM_ALIGNTEXT, (WPARAM)dwAlignment, 0)); 
		}
		inline BOOL SetMarquee(BOOL bOn, UINT uMsecBetweenUpdate)	//感觉就是让进度条进行滚动
			{ 
				ASSERT(::IsWindow(m_hWnd)); 
				return ((BOOL) ::SendMessage(m_hWnd, PBM_SETMARQUEE, (WPARAM)bOn, (LPARAM)uMsecBetweenUpdate)); 
		}
		inline int SetMarqueeOptions(int nBarSize)			//选项滚动模式？
			{ 
				ASSERT(::IsWindow(m_hWnd)); 
				return ((BOOL) ::SendMessage(m_hWnd, PBM_SETMARQUEEOPTIONS, (WPARAM)nBarSize, 0)); 
		}
