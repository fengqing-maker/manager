...
BOOL CInPlaceList::AttachToGridCell(CGridCell* pCell)
{
	m_pCell = pCell;
	return TRUE;
}
BOOL CInPlaceList::DetachFromGridCell()
{
	m_pCell = 0;
	return TRUE;
}
BOOL CInPlaceList::IsGridCellValid()
{
	if (m_pCell != 0)
	{
		return TRUE;
...
CGridCellCombo::~CGridCellCombo()
{
	...
	if (m_pEditWnd)
		((CInPlaceList*)m_pEditWnd)->DetachFromGridCell();
}
...
BOOL CGridCellCombo::Edit(int nRow, int nCol, CRect rect, CPoint /* point */, UINT nID, UINT nChar)
{
	...
	if (m_pEditWnd)		/* 列表框创建成功后与表项进行关联，关联后列表框才可以对表项进行编辑 */
		((CInPlaceList*)m_pEditWnd)->AttachToGridCell(this);
    return TRUE;
}
...
BOOL CGridCellCombo::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
    ...
    if ( pGrid->IsCellEditable(nRow, nCol) && !IsEditing())
    {
