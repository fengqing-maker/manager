...
int  GetStringLines( const CString &str, int &cnt );
                     ...
                     COLORREF crBackClr /* CLR_DEFAULT */)
{
    ...
    //lhz add by 2015-5-14 
	int cnt = 0;
	int maxLen = GetStringLines( strTitle, cnt );
	rectDisplay.bottom += cnt*size.cy;
	//end
    ...
    if ( rectDisplay.right > rectTitle.right-xoffset )
    {
		if ( cnt > 0 )
		{//lhz add by 2015-5-14 
			rectDisplay.right = (rectDisplay.right - size.cx) + (tm.tmMaxCharWidth/*+tm.tmAveCharWidth*/)*maxLen/*/2*/;
		}
		// Show the titletip
       ...
       // dc.TextOut( 0, 0, strTitle ); 
		//lhz moidy by 2015-5-14 
		CRect rect;
		dc.GetClipBox(&rect);     // Erase the area needed 
		dc.DrawText( strTitle, -1, rect, DT_WORDBREAK );
		//end
        SetCapture();
    }
...
int  GetStringLines( const CString &str, int &cnt )
{
	...
	while( pos >= 0 )
	{
		...
		pos = tmpStr.Find( _T("\n") );
		subStr = tmpStr.Left( pos );
		cnt++;
		int tmpLen = subStr.GetLength();
		if ( 0 == tmpStr.GetLength() )
		{
			cnt--;
		}
		tmpLen -= 1;
	...
	if ( tmpStr.GetLength() > maxLen )
	{
		maxLen = tmpStr.GetLength();
	}
	return maxLen;
}
