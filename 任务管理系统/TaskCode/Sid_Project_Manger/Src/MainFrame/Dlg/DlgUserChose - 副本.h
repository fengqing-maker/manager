...
public:
	CDlgUserChose(BOOL isMutilSel,DEPARTTYPE type = choseUser,CWnd* pParent = NULL);   // 标准构造函数
	virtual ~CDlgUserChose();

	void  SetDepartException( int departId );
	void  SetDelSelUser( const ListUser &defUser );
	void  SetDelSelUser( int id );
	void  SetDefDepart( int departId );
...
protected:
	...
	HTREEITEM IsInSelTree( int id, TreeValue &value  );
	...
	const CDepartment*  FindDepartById( int departId );
