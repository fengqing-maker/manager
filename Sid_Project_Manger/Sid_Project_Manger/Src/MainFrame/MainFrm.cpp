...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	//提示进度框，连接时有点慢
    m_progressThread.create();
	m_progressThread.addObserver( this->GetSafeHwnd() );
	m_progressThread.ShowMsgDlg( MSG_CONNECT_SEVER );
	if ( FALSE == m_actionFactory.GetConnectSever()->Connect()
		 || FALSE == m_watchFactory.GetConnectSever()->Connect())
	{
		m_progressThread.CloseMsgDlg();
        AfxMessageBox( _T("服务器连接失败，请找服务器管理员") );
		return -1;
	}
...
void CMainFrame::ShowOutput( BOOL isShow )
{
	...
	{//不可见到可见
		...
		CPaneDivider* pDefaultSlider = m_wndOutput.GetDefaultPaneDivider();
...
void CMainFrame::ShowMenu()
{
	...
	int removeCount = 0;
    if( FALSE == pLoginAction->FindLoginPower( projctTaskManger ) )
	{
		removeCount++;
		m_wndMenuBar.RemoveButton( 1 );
	}
	if( FALSE == pLoginAction->FindLoginPower( departTaskManger ) )
	{
		m_wndMenuBar.RemoveButton( 2-removeCount );
		removeCount++;
	}
	if( FALSE == pLoginAction->FindLoginPower( backgroundManger ) )
	{
...
void CMainFrame::LoginLoading()
{
	...
	if ( userExcOk == ret )
	{
		const CUser &obj = m_actionFactory.GetLoginServer()->GetLoginUser();
		m_actionFactory.GetTaskSever()->SetLoginUser( obj );
		...
		m_wndOutput.SetActionFactory( &m_actionFactory );
