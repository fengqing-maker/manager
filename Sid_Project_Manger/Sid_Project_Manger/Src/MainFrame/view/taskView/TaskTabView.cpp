...
int  CTaskTabView::OnCreateTab()
{
	...
	if ( AddView( RUNTIME_CLASS(CMyTaskView), GetTabString(MyTask, handCnt ),count, &context ) < 0 )
	{
		TRACE("我的任务Table创建失败.\n");
		return -1;
	}
	...
	/* 创建任务发布视图 */
	BOOL enable = m_pSever->GetLoginServer()->FindLoginPower( taskAgree ) 
		|| m_pSever->GetLoginServer()->FindLoginPower( ReviewProjectTaskFinish );
