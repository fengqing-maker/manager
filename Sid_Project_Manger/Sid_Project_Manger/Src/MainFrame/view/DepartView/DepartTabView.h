...
class CDepartTabView : public CTabView
{
	DECLARE_DYNCREATE(CDepartTabView)

protected:
	CDepartTabView();           // 动态创建所使用的受保护的构造函数
	virtual ~CDepartTabView();
public:
...
public:
		/** @brief 选项卡序号 */
	//部门经理，总经理，主管
	enum Tab_Index{ departTabIndex, departTaskTabIndex, schedulePictrue};
	/**
	*@brief	激活指定Tab页面
...
protected:
	...
	afx_msg LRESULT OnChangeDepartTaskView( WPARAM wPara, LPARAM lPara );
