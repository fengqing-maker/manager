...
{
	//ListTask taskList;
	//BOOL ret = m_pTaskDbDAo->GetTaskByOwernerId( userId, ongoing, taskList );
	//if ( userExcOk != ret )
	//{
	//	return ret;
	//}
	//ret = m_pTaskDbDAo->GetTaskByOwernerId( userId, task_suspend, taskList );
	//if ( userExcOk != ret )
	//{
	//	return ret;
	//}
	//ret = m_pTaskDbDAo->GetTaskByOwernerId( userId, midStop, taskList );
	//if ( userExcOk != ret )
	//{
	//	return ret;
	//}
	//ret = m_pTaskDbDAo->GetTaskByOwernerId( userId, changeWait1, taskList );
	//if ( userExcOk != ret )
	//{
	//	return ret;
	//}
	//ret = m_pTaskDbDAo->GetTaskByOwernerId( userId, changeWait2, taskList );
	//if ( userExcOk != ret )
	//{
	//	return ret;
	//}
	//ListTaskItor itor = taskList.begin();
	//while( itor != taskList.end() )
	//{
	//	m_pChangeDbDao->GetChangeItemsByTaskId( itor->GetId(), list );
	//	itor++;
	//}

	BOOL ret = m_pChangeDbDao->GetChangerItemsByTaskOwner( userId, list );
	if ( ret != userExcOk )
	{
		return ret;
	}
	m_ownChangeCount = list.size();
	ret = m_pChangeDbDao->GetChangerItemsByProjectManger( userId, list); //2015-0-26 add 项目经理点评的变更
	ret = m_pChangeDbDao->GetChangerItemsByReviewUser( userId, list );
	return ret;
}
...
BOOL CTaskSeverImp::UpdateOwernTask()
{
	...
	BOOL ret = m_pTaskDbDAo->GetNewingTaskByCreateId( userId, m_ownerTaskList ) ;
	if ( userExcOk != ret )
	{
		return ret;
	}
	m_newingTaskCount = m_ownerTaskList.size();
	ret = m_pTaskDbDAo->GetTaskByOwernerId( userId, ongoing ,m_ownerTaskList ) ;
	if ( userExcOk != ret )
	{
		return ret;
	}
	m_goingTaskCount = m_ownerTaskList.size();
	ret = m_pTaskDbDAo->GetTaskByOwernerId( userId, waitStart,m_ownerTaskList);
	return ret;
}
...
void CTaskSeverImp::SetLoginUser( const CUser &obj )
{
	...
	m_loginUser = obj;
	m_loginPower.clear();
	m_pDbDao->GetUserDao()->GetUserPowersByRoleId( m_loginUser.GetRolId(), m_loginPower );
	m_stausMap.Clear();
	m_pTaskDbDAo->GetTaskStatusMap( m_stausMap );
	...
	if ( m_loginUser.GetId() >= 0  )
	{
		...
		while( itor != m_loginPower.end() )
		{
			...
			case taskAgree:
				...
				break;
				//case  projectReview:
				//	UpdatePrjReviewSchedule();
				//	break;
				//case  changeLevelAgree1:
				//	UpdateReviewChange();
				//	break;
				//case changeLevelAgree2:
				//	UpdateGernalChange();
			}
			itor++;
		}
