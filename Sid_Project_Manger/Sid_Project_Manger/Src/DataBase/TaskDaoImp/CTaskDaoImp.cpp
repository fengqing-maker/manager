...
BOOL CTaskDaoImp::GetTaskByOwernerId( int owernId, EnumStaus status, ListTask &listObj, int year /*= 0*/, int quater /*= 0*/)
{
	...
	if ( status > 0)
	{
		str.Format( _T("select * from %s where task_ownerid = %d and task_statusid = %d and task_attri >= 0") , m_taskTableName, owernId, status );
	}else
	{
		str.Format( _T("select * from %s where task_ownerid = %d  and task_attri >= 0") , m_taskTableName, owernId );
	}
