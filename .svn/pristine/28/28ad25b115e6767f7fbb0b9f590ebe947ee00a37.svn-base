...
// **************************
//
// Written by Mathieu DUBA薒E (mathieu@appstmd.com)
//
					...
					LONG *pcb)
{
	...
	//lhz moidy 修改适合用多字节工程
    long len = strlen( (char *)pstr->GetBuffer() );
	if( len < cb )
	{
		*pcb = len;  
		//WideCharToMultiByte(CP_ACP, 0,  *pstr,-1, (LPSTR)pbBuff, len, NULL, NULL);
...
BOOL CGridCellRich::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
	...
	if (Text.Left (6)=="{\\rtf1")
		...
		StreamType=SF_TEXT; //l11 lhzmoidy
		//StreamType = SF_UNICODE;
