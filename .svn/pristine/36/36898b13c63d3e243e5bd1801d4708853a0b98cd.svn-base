                           ...
                           UINT nFirstChar)
{
    ...
    DWORD dwEditStyle = WS_BORDER|WS_CHILD|WS_VISIBLE| ES_AUTOHSCROLL |ES_MULTILINE | ES_AUTOVSCROLL
        | dwStyle;
...
void CInPlaceEdit::EndEdit()
{
	if (!IsGridCellValid())
	{
		this->DestroyWindow();	/* 与编辑框控件关联的表项无效时仅销毁窗口 */
		return;
	}
...
BOOL CInPlaceEdit::AttachToGridCell(CGridCell* pCell)
{
	m_pCell = pCell;
	return TRUE;
}
BOOL CInPlaceEdit::DetachFromGridCell()
{
	m_pCell = 0;
	return TRUE;
}

BOOL CInPlaceEdit::IsGridCellValid()
{
	if (m_pCell != 0)
	{
		return TRUE;
