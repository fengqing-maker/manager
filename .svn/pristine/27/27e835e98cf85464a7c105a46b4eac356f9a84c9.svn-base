...
void CGridCtrl::OnDraw(CDC* pDC)
{
	...
	CGridCellBase *pCellTmp;
	CRect removelinerect;
	//lhzabove is added for combine cell
    ...
    if (m_nGridLines == GVL_BOTH || m_nGridLines == GVL_VERT)
    {
        ...
        for (col = minVisibleCol; col <= maxVisibleCol; col++)
        {
			...
			//lhz above is added for combine;
        }
    }
	...
	pDC->SetROP2(orientedROP);//lhz added for combine;
...
BOOL CGridCtrl::RedrawCell(int nRow, int nCol, CDC* pDC /* = NULL */)
{
    ...
    if (pDC)
    {
        ...
        else
        {
			...
			//lhz  above added for combine cell;
            if (pCell)
                bResult = pCell->Draw(pDC, nRow, nCol, rect, TRUE);
								...
								BOOL bSelectPastedCells /*=TRUE*/)
{
    ...
    do
    {
        ...
        while (!strLine.IsEmpty())
        {
			...
			//CGridCellBase *pCell;//lhz added for combine
			//pCell=GetCell(cell.row + nLine, cell.col + nColumn);//lhz added for combine
            if (IsValid(TargetCell) /*&& !pCell->m_bNoEdit*/)//lhz  added (&&!pCell->m_bNoEdit)for combine
            {
...
BOOL CGridCtrl::MouseOverRowResizeArea(CPoint& point)
{
	...
	CGridCellBase *pCell,*pTheTopLeft;
	...
	//lhz added for combine the fixdecell;
    CPoint start;
...
/*virtual*/ void CGridCtrl::DestroyCell(int nRow, int nCol)
{
	...
	CGridCell* pCell = static_cast<CGridCell*>(GetCell(nRow, nCol));
	if (pCell != 0)
	{
		delete pCell;
	}
...
void CGridCtrl::OnMouseMove(UINT /*nFlags*/, CPoint point)
{
        ...
        (m_bLMouseButtonDown && m_MouseMode == MOUSE_NOTHING))
    {
        ...
        if (m_MouseMode == MOUSE_NOTHING)
        {
			...
			bool isNotCombin = true;
			if ( pCell )
			{
				isNotCombin = pCell->m_bHide!=TRUE&& pCell->m_numHhide==0&&pCell->m_numVhide==0;
			}
			//lhz above added for combine;
            if (m_bTitleTips && isNotCombin)
            {
...
BOOL CGridCtrl::ValidateEdit(int nRow, int nCol, LPCTSTR str)
{
    ...
    BOOL result = FALSE;
	if (SendMessageToParent(nRow,nCol,GVN_VALIDATECELL) >=0 
...
void CGridCtrl::SetCellCombine(int nRow, int nCol, int xNum, int yNum,BOOL bNoEdit)//设置单元格的合并特性
	...
	//欢迎使用和改进。
{
	//xNum=(int)fabs((float)xNum);
	//yNum=(int)fabs((float)yNum);
...
void CGridCtrl::UndoCellCombine(int nRow, int nCol)
{
	if(!IsValid(nRow,nCol))
		return;
	CGridCellBase *pcellold,*pCell;
	...
	for ( i=0;i<=xNum;i++)
		...
		for ( i=0;i<=xNum;i++)
			...
			for( j=0;j<=yNum;j++)
			{
				if(i==0&&j==0) continue;
				pCell=NULL;
				pCell=(CGridCell *)GetCell(nRow+i,nCol+j);
				pCell->m_bHide=TRUE;
				pCell->m_TopleftCol=nCol;
				pCell->m_TopleftRow=nRow;
...
CGridCell* CGridCtrl::GetCellP(int nRow, int nCol)
{
	...
	GRID_ROW* pRow = m_RowData[nRow];
	if (!pRow) return NULL;
	return (CGridCell *)pRow->GetAt(nCol);
}