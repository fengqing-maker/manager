...
CCellID CGridCtrl::GetCellFromPt(CPoint point, BOOL bAllowFixedCellCheck /*=TRUE*/)
{
    ...
    int fixedColWidth = GetFixedColumnWidth();				//获取表格的总列宽
    ...
    // calculate row index
    int fixedRowHeight = GetFixedRowHeight();			//表格总行高
    ...
    else
    {
        ...
        while ( row < GetRowCount() )			//在表格总行数目以内的话
        {
            ypos += GetRowHeight(row);
            if (ypos > point.y)
                break;
...
// Gets the first non-fixed cell ID			通过滚动条位置，求出行与列第一个完全裸露在界面上的表格行列索引
CCellID CGridCtrl::GetTopleftNonFixedCell(BOOL bForceRecalculation /*=FALSE*/)
{
    ...
    m_idTopLeftCell.col = m_nFixedCols;			//获取固定的列数目
    int nRight = 0;
	//函数的意思是获取列宽
    while (nRight < nHorzScroll && m_idTopLeftCell.col < (GetColumnCount()-1))	//意思是越过固定的列宽的那一列
        nRight += GetColumnWidth(m_idTopLeftCell.col++);
	//固话的行数
    m_idTopLeftCell.row = m_nFixedRows;			//固定的行
    int nTop = 0;
...
BOOL CGridCtrl::AutoSizeRow(int nRow, BOOL bResetScroll /*=TRUE*/)
{
   ...
   // int nHeight = 0;
	int nHeight = GetRowHeight( nRow); //lhz 2015-5-15 moidy
...
//BOOL  CGridCtrl::SetItemMutiRow(int nRow, int nCol)
//{
//	if ( GetVirtualMode() )
//	{
//		return FALSE;
