...
CGridCell::~CGridCell()
{
    delete m_plfFont;
	if (m_pEditWnd)
		((CInPlaceEdit*)m_pEditWnd)->DetachFromGridCell();
}
...
void CGridCell::SetFont(const LOGFONT* plf)
{
    ...
    {
        if (!m_plfFont)				//如果字体属性不存在就创建
            m_plfFont = new LOGFONT;
        if (m_plfFont)				//把传入的字体格式属性拷贝
...
BOOL CGridCell::Edit(int nRow, int nCol, CRect rect, CPoint /* point */, UINT nID, UINT nChar)
{
	...
	{   
		...
		if (GetFormat() & DT_RIGHT) 
			...
			dwStyle = ES_CENTER;
	//	 dwStyle |= ES_MULTILINE;//lhz  2015-4-7 moidy
		m_bEditing = TRUE;
		...
		if(m_pEditWnd)
			((CInPlaceEdit*)m_pEditWnd)->AttachToGridCell(this);
    }
    return TRUE;
}
