...
{
	m_bModel = isModel;		
	if (isModel == true)		//如果是模式对话框，直接使用DoModal进行显示的对话框
	{
		m_bCreated = TRUE;
...
/******************************private function*****************************/
//初始化进度条对话框的进度条样式，和进度条显示方式
BOOL CProgressDialog::InitControls()
{
	m_bCanceled = FALSE;
	/* 设置图标 */
	HICON m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);			//使用到这个应用的图标
	...
	if (m_content.bCancelButtonVisible == FALSE)
	{
		...
		CMenu* pSysMenu = GetSystemMenu(FALSE);			//如果参数bRevert为FALSE，返回值是窗口菜单的拷贝的句柄：如果参数bRevert为TRUE，返回值是NULL。
